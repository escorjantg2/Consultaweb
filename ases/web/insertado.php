...
$alumno="SELECT count(*) as cuenta from asistencia where Alumnos_idalumnos='".$_SESSION['alumno'][$j]."' and fecha='".$_SESSION['fecha'][$j]."' and asignatura_idasignatura='".$_SESSION['asignatura'][$j]."'";
$ejec=mysql_query($alumno,$conexion);
$contador[$j]=mysql_fetch_array($ejec);

if($contador[$j]['cuenta']>0)
{
}
else
{
...
if($falta=="on")
{
$insf = "INSERT into asistencia(Alumnos_idalumnos,asignatura_idasignatura,fecha,falta,observaciones)
values (".$_SESSION['alumno'][$j].",".$_SESSION['asignatura'][$j].",'".$_SESSION['fecha'][$j]."','Si','".$_REQUEST['observacionesex'][$j]."')";
$ejecf = mysql_query($insf,$conexion);
$q++;
}
}
if(isset($_POST['retraso'][$j]))
{
$retraso = $_POST['retraso'][$j];
if($retraso=="on")
{
$insr = "INSERT into asistencia(Alumnos_idalumnos,asignatura_idasignatura,fecha,tarde,observaciones)
values (".$_SESSION['alumno'][$j].",".$_SESSION['asignatura'][$j].",'".$_SESSION['fecha'][$j]."','Si','".$_REQUEST['observacionesex'][$j]."')";
$ejecr = mysql_query($insr,$conexion);
$q++;
}
}
}
...
<thead> 
   ...
   <!-- esto son la cantidad de columnas que tendra el grid         <th width="auto">Titulo 7</th> -->
    ...
    <tbody> 
    <?php 
$grid="select a.nombre as nombrealumno, a.apellido1, a.apellido2, asig.nombre as nombreasignatura,c.nombre as nombreciclo,asis.falta, asis.tarde, Justificado, asis.fecha as fecha, asis.observaciones as oba
...
echo "<br>";
       ...
       while ($row = mysql_fetch_array($mysql)) { 
			...
			<td width='auto'>".$row['tarde']."</td> 
			<td width='auto'>".$row['Justificado']."</td>
			<td width='auto'><form name='observacion' action='observaciones.php'method='post' target='_blank' align='center'><input type='hidden' name='observacion' value='".$row['oba']."'/><input type='submit' name='enviarob' value='Ver'/></form></td>
            </tr>"; 
       }	
       $i++;
       } ?>
